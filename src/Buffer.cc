...
#include <Buffer.h>
//在非阻塞 I/O 中，应用层缓冲区是必不可少的。如果你直接拿一个定长的 char array[1024] 来接数据，极其容易发生数据截断或溢出；如果频繁 new/delete，又会造成严重的内存碎片。
/**
...
//readfd是从fd上读取数据 writefd是通过fd发送数据 也就是把Buffer中的数据写入到fd上，而buffer上的有用的数据就是从readerIndex_到writerIndex_之间的数据 也就是peek()函数返回的地址开始的长度为readableBytes()的数据
