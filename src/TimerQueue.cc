                          ...
                          double interval)
{
    ...
    // 线程安全地在EventLoop所在的线程中添加定时器，将addTimerInLoop方法和定时器对象绑定成一个函数对象，传入EventLoop的runInLoop方法中执行
    loop_->runInLoop(
...
std::vector<TimerQueue::Entry> TimerQueue::getExpired(Timestamp now)
{
    ...
    TimerList::iterator end = timers_.lower_bound(sentry);// lower_bound返回第一个不小于sentry的迭代器
    std::copy(timers_.begin(), end, back_inserter(expired));// 将过期的定时器节点添加到expired中
    timers_.erase(timers_.begin(), end);//从定时器队列中删除过期的定时器节点
...
void TimerQueue::handleRead()
{
    ...
    ReadTimerFd(timerfd_);// 把闹钟数据读出来，否则 epoll 会一直报警（LT 模式）
